...
def create_foreclosure_profile(db: Session, *, case_id: UUID, payload: dict, actor_id: UUID | None) -> ForeclosureCaseData:
    profile = db.query(ForeclosureCaseData).filter(ForeclosureCaseData.case_id == case_id).first()
    if profile:
        raise HTTPException(status_code=409, detail="Foreclosure profile already exists for case")

    profile = ForeclosureCaseData(case_id=case_id, **payload)
    db.add(profile)
    db.flush()

    _audit(db, actor_id=actor_id, case_id=case_id, action_type="foreclosure_profile_created", reason_code="foreclosure_profile_created", after_state={"profile_id": str(profile.id)})
    return profile
