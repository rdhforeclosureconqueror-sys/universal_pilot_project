...
def ingest_leads(db: Session, *, source_name: str, source_type: str, leads: list[dict]) -> dict:
    ...
    ingested = 0
    for lead in leads:
        if deduplicate_leads(db, source_id=source.id, property_address=lead.get("property_address", "")):
            continue
        db.add(
            PropertyLead(
                source_id=source.id,
        ...
        ingested += 1
    db.flush()
    return {"source": source_name, "leads_ingested": ingested}


def deduplicate_leads(db: Session, *, source_id: UUID, property_address: str) -> bool:
    existing = (
        db.query(PropertyLead)
        .filter(PropertyLead.source_id == source_id, PropertyLead.property_address == property_address)
        .first()
    )
    return existing is not None


def score_property_lead(db: Session, *, lead_id: UUID) -> dict:
    ...
    score_row = LeadScore(lead_id=lead.id, score=score, grade=grade, recommended_action="create_case" if score >= SCORE_THRESHOLD else "monitor")
    db.add(score_row)
    db.flush()

    created_case_id = None
    if score >= SCORE_THRESHOLD:
        created_case_id = create_case_from_lead(db, lead_id=lead.id)
        score_row.created_case_id = created_case_id
        db.flush()

    return {"lead_id": str(lead.id), "score": score, "grade": grade, "created_case_id": str(created_case_id) if created_case_id else None}


def create_case_from_lead(db: Session, *, lead_id: UUID) -> UUID:
    lead = db.query(PropertyLead).filter(PropertyLead.id == lead_id).first()
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found")
...
def weekly_foreclosure_scan(db: Session) -> dict:
    ...
    return {"status": "success", "weekly_leads_ingested": total}
...
def dallas_county_foreclosure_connector(db: Session) -> dict:
    leads = [
        {"property_address": "100 Elm St", "city": "Dallas", "state": "TX", "foreclosure_stage": "pre_foreclosure", "tax_delinquent": True, "equity_estimate": 60000},
    ]
    return ingest_leads(db, source_name="dallas_county_foreclosure_connector", source_type="foreclosure_filings", leads=leads)


def tarrant_county_trustee_connector(db: Session) -> dict:
    leads = [
        {"property_address": "200 Oak St", "city": "Fort Worth", "state": "TX", "foreclosure_stage": "auction_scheduled", "tax_delinquent": False, "equity_estimate": 45000},
    ]
    return ingest_leads(db, source_name="tarrant_county_trustee_connector", source_type="trustee_sale_notices", leads=leads)


def collin_county_notice_connector(db: Session) -> dict:
    leads = [
        {"property_address": "300 Pine St", "city": "Plano", "state": "TX", "foreclosure_stage": "notice_of_default", "tax_delinquent": True, "equity_estimate": 85000},
    ]
    return ingest_leads(db, source_name="collin_county_notice_connector", source_type="notice_connector", leads=leads)
